  ...
  createPiBakeryDirectory(mountpoint, function (error) {
    ...
    modifyCmdlineTxt(mountpoint, function (error) {
      ...
      writeBootScripts(mountpoint, data, function (error) {
        ...
        setTimeout(getJEDIOneDownloadURL(mountpoint, cb), 10000)
      })
...
function getJEDIOneDownloadURL(mountpoint, cb) {
  var pibakeryDir = path.join(mountpoint, 'PiBakery')
  var downloadjediJSON = pibakeryDir + '/blocks/downloadjedi/downloadjedi.json'

  var exists = fs.existsSync(downloadjediJSON)
  // if settings file doesn't exist, try and create it
  if (exists) {
    fs.readJson(downloadjediJSON, function (error, data) {
      if (error) {
        cb.error(err)
        return
      }
      downloadJediOne(mountpoint, cb, data.args[0].default + data.args[1].default);
    })
  }else{
    cb.done()
  }
...
function downloadJediOne(mountpoint, cb, url) {
  cb.progress({ text: 'Downloading JEDI One...' })
  var pibakeryDir = path.join(mountpoint, 'PiBakery')
  var source = url
  var destination = pibakeryDir + '/pibakery-raspbian/opt/jedione'
  ...
  download(source, options, function (err) {
    if (err) {
      cb.error(err)
      return
    }
